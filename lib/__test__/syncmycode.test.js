...
describe("Local Sync", function () {
    ...
    it("should parse config", function () {
        config = config_1.loadConfig(sourcePath + "/sync.json");
    ...
    var buttonLocalPath = sourcePath + "/lib/some-ui-library/Button.js";
    ...
    it("should detect stale changes", function () {
        var code = fs_1["default"].readFileSync(buttonSourcePath, "utf-8");
        var updatedCode = code.replace("// DONE: ", "// FINSIHED: ");
        fs_1["default"].writeFileSync(buttonSourcePath, updatedCode, "utf-8");
        var diffs = sync_1.diffLocalChanges(config.syncers[0]);
        expect(diffs.length).toEqual(1);
        expect(diffs[0].isStale).toEqual(true);
    });
    it("should generate a workspace", function () {
        var workspace = helpers_1.createVSCodeWorkspace(config.syncers);
        expect(workspace.folders.length).toEqual(2);
    });
});
//# sourceMappingURL=syncmycode.test.js.map